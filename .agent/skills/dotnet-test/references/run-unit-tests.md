...
The pre-commit workflow runs tests automatically:

```yaml
# .github/workflows/pre-commit.yml
- run: cd dotnet && dotnet test shared-app.Tests/PigeonPea.Shared.Tests.csproj --no-restore --verbosity normal
```
...
PigeonPea test projects follow xUnit conventions:

- **Test files:** Named `{Class}Tests.cs` (e.g., `FrameTests.cs`)
- **Test methods:** Decorated with `[Fact]` or `[Theory]` attributes
- **Assertions:** Use xUnit assertions (`Assert.Equal`, `Assert.True`, etc.) or FluentAssertions
- **Mocking:** Use Moq for dependency mocking
- **Test data:** Use `[InlineData]`, `[MemberData]`, or `[ClassData]` for parameterized tests
...
public class FrameTests
{
    ...
    [InlineData(80, 24)]
    [InlineData(120, 40)]
    public void ShouldCreateFrameWithDimensions(int width, int height)
    {
...
2. Output shows "Passed! - Failed: 0"
3. All test projects discovered and executed
4. No test crashes or timeouts

## Related Procedures

- **Generate coverage:** See [`generate-coverage.md`](generate-coverage.md)
- **Run benchmarks:** See [`run-benchmarks.md`](run-benchmarks.md)
- **Build before testing:** Use `dotnet-build` skill
