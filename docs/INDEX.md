# Documentation Index
*Generated: 2025-11-20 08:06:16*
*Total Documents: 60*

## Quick Navigation
- [By Document Type](#by-document-type)
- [By Status](#by-status)
- [Recently Updated](#recently-updated)
- [Most Referenced](#most-referenced)
- [By Topic](#by-topic)

## By Document Type

### RFCs (Requests for Comments) (27)
- 📦 **[](_inbox\023-nexus-input-system.md)**: Nexus-Input: Unity-Inspired Input System
- 📝 **[](_inbox\024-nexus-camera2d-system.md)**: Nexus-Camera2D: ProCamera2D-Inspired 2D Camera System
- 📝 **[](_inbox\025-nexus-inventory-system.md)**: Nexus-Inventory: RPGCore-Inspired Inventory and Item System
- 📝 **[RFC-00001](rfcs\001-rendering-architecture.md)**: Rendering Architecture
- 📝 **[RFC-00003](rfcs\003-testing-verification.md)**: Testing and Verification
- 📝 **[RFC-00004](rfcs\004-agent-infrastructure-enhancement.md)**: Agent Infrastructure Enhancement
- ✅ **[RFC-00005](rfcs\005-project-structure-reorganization.md)**: Project Structure Reorganization
- ✅ **[RFC-00006](rfcs\006-plugin-system-architecture.md)**: Plugin System Architecture
- 📝 **[RFC-00012](rfcs\012-documentation-organization-management.md)**: Documentation Organization Management
- 📝 **[RFC-00013](rfcs\013-plugin-architecture-refinement-tiered.md)**: Plugin Architecture Refinement: Tier-Based System
- 📝 **[RFC-00014](rfcs\014-scene-management-ecs.md)**: Scene Management with ECS Architecture
- 📝 **[RFC-00015](rfcs\015-fantasy-calendar-real-world-transformation.md)**: Fantasy Calendar to Real-World Time Transformation
- 📝 **[RFC-00016](rfcs\016-name-generator-json-configuration.md)**: Name Generator JSON Configuration System
- 📝 **[RFC-00017](rfcs\017-name-generator-cjk-languages.md)**: Name Generator CJK Language Support
- 📝 **[RFC-00018](rfcs\018-name-generator-markov-chain-mode.md)**: Name Generator Markov Chain Mode
- ✅ **[RFC-00019](rfcs\019-nexus-gas-gameplay-ability-system.md)**: Nexus-GAS: Gameplay Ability System Implementation
- 📝 **[RFC-00020](rfcs\020-font-pipeline-and-pua-layout.md)**: Custom CJK Monospace Game Font — Pipeline and PUA Layout
- 📝 **[RFC-00021](rfcs\021-terminal-distribution-and-rio-integration.md)**: Terminal Distribution — Rio Integration and Bundle Layout
- 📝 **[RFC-00022](rfcs\022-ci-cd-font-and-bundle.md)**: CI/CD for Custom Font and Developer Bundle
- 📝 **[RFC-00023](_inbox\game-services-architecture.md)**: Game Services Architecture: Stats, Character, Avatar, Animation, Persistence, and World Management
- 📝 **[RFC-00023](_inbox\rfc-023-input-system-architecture-refactoring.md)**: Input System Architecture Refactoring: Tier-Based System and Platform Plugins
- 📝 **[RFC-00024](_inbox\rfc-024-nexus-systems-architecture-refactoring.md)**: Nexus Systems Architecture Refactoring: Move to Engine Layer
- 📝 **[RFC-00024](_inbox\stats-service-rfc.md)**: Stats Service: Universal Stat Management for Game Entities
- 📝 **[RFC-00025](_inbox\world-management-service-rfc.md)**: World Management Service: Multiple ECS Worlds for Interpolation, Simulation, and Scene Management
- 📝 **[RFC-00026](_inbox\rfc-numbering-convention.md)**: RFC Numbering Convention and Document Identification
- ✅ **[RFC-00027](rfcs\027-nexus-goap-ai-system.md)**: Nexus-GOAP: Goal-Oriented Action Planning AI System
- ✅ **[RFC-00028](rfcs\028-nexus-perception-system.md)**: Nexus-Perception: Agent Perception and Awareness System

### Guides (9)
- 📝 **[](_inbox\mcp-project-launcher-system.md)**: MCP Project Launcher System
- ✅ **[GUIDE-00001](guides\architecture-refinement-implementation-guide.md)**: Architecture Refinement Implementation Guide
- ✅ **[GUIDE-00001](guides\name-generator-enhancement-guide.md)**: Name Generator Enhancement Implementation Guide
- ✅ **[GUIDE-00001](dotnet\guides\getting-started.md)**: Getting Started with Pigeon Pea .NET
- ✅ **[GUIDE-00002](dotnet\NAVIGATION.md)**: .NET Documentation Navigation Guide
- ✅ **[GUIDE-00002](guides\dotnet-tiered-architecture-guide.md)**: .NET Tiered Architecture and Layer Implementation Guide
- ✅ **[GUIDE-00002](guides\rfc014-scene-management-adjustment-plan.md)**: RFC-014 Scene Management: Adjustment Plan
- 📝 **[GUIDE-00002](_inbox\game-services-implementation-guide.md)**: Game Services Implementation Guide: Step-by-Step Instructions for Agents
- ✅ **[GUIDE-00002](terminals\rio\CONFIG.md)**: Rio Terminal Configuration — Using the Custom Font

### Architecture Decision Records (7)
- ✅ **[ADR-00001](dotnet\architecture\overview.md)**: Architecture Overview
- ✅ **[ADR-00002](dotnet\architecture\game-scale-modes.md)**: Multi-Scale World & Mode System
- ✅ **[ADR-00003](dotnet\architecture\service-tiers.md)**: Service Tiers and Category Layout
- ✅ **[ADR-00004](dotnet\architecture\services-and-plugins.md)**: Services, Shared Libraries, and Plugins
- ✅ **[ADR-00005](dotnet\architecture\goap-perception-checklist.md)**: GOAP Perception Checklist
- ✅ **[ADR-00006](dotnet\architecture\observable-collections.md)**: Observable Collections and Reactive Patterns
- ✅ **[ADR-00007](dotnet\architecture\reactive-stack-and-r3.md)**: Reactive Stack and R3 Integration

### Planning Documents (13)
- ✅ **[PLAN-00003](rfcs\issues\issue-001-migrate-structure.md)**: Issue #1: [RFC-005] Phase 1: Migrate project structure to new organization
- ✅ **[PLAN-00004](rfcs\issues\issue-002-create-contracts.md)**: Issue #2: [RFC-005] Phase 2: Create contract projects for plugin system
- ✅ **[PLAN-00005](rfcs\issues\issue-003-plugin-system.md)**: Issue #3: [RFC-006] Phase 1: Implement core plugin infrastructure
- ✅ **[PLAN-00006](rfcs\issues\issue-004-game-events.md)**: Issue #4: [RFC-006] Phase 2: Integrate game events with plugin system
- ✅ **[PLAN-00007](rfcs\issues\issue-005-rendering-plugin.md)**: Issue #5: [RFC-006] Phase 3: Create rendering plugin proof of concept
- ✅ **[PLAN-00008](planning\AGENT_INFRASTRUCTURE_ISSUES.md)**: GitHub Issues: Agent Infrastructure Enhancement
- ✅ **[PLAN-00009](planning\CODE_REVIEW_AGENT_INFRASTRUCTURE.md)**: Code Review: Agent Infrastructure Implementation
- ✅ **[PLAN-00010](planning\ENHANCEMENT_PLAN.md)**: Pigeon Pea Enhancement Plan
- ✅ **[PLAN-00011](planning\GITHUB_ISSUES.md)**: GitHub Issues Breakdown
- ✅ **[PLAN-00012](planning\LABEL_STRATEGY.md)**: GitHub Label Strategy
- ✅ **[PLAN-00013](planning\RFC_004_EXECUTION_ORDER.md)**: RFC-004 Implementation: Execution Order
- ✅ **[PLAN-00014](planning\SELF_REVIEW_RFC_004_PLANNING.md)**: Self-Review: RFC-004 & Issues Planning Quality
- ✅ **[PLAN-2025-00001](rfcs\IMPLEMENTATION_PLAN.md)**: Implementation Plan: Project Restructure and Plugin System

### Specifications (1)
- ✅ **[SPEC-00001](fonts\SVG-SPEC-ANGLES.md)**: SVG Specification — 36 Angle Glyphs

### Reference Documentation (3)
- ✅ **[REFERENCE-00001](DOCUMENTATION-SCHEMA.md)**: Documentation Front-Matter Schema
- ✅ **[REFERENCE-00002](fonts\ANGLE-MAPPING-EXAMPLES.md)**: Angle → Glyph Mapping Examples
- ✅ **[REFERENCE-00003](terminals\rio\BUNDLE-LAYOUT.md)**: Rio Bundle Layout (Template)

## By Status

### Active (36)
- [ADR-00001](dotnet\architecture\overview.md) - Architecture Overview *(adr)*
- [ADR-00002](dotnet\architecture\game-scale-modes.md) - Multi-Scale World & Mode System *(adr)*
- [ADR-00003](dotnet\architecture\service-tiers.md) - Service Tiers and Category Layout *(adr)*
- [ADR-00004](dotnet\architecture\services-and-plugins.md) - Services, Shared Libraries, and Plugins *(adr)*
- [ADR-00005](dotnet\architecture\goap-perception-checklist.md) - GOAP Perception Checklist *(adr)*
- [ADR-00006](dotnet\architecture\observable-collections.md) - Observable Collections and Reactive Patterns *(adr)*
- [ADR-00007](dotnet\architecture\reactive-stack-and-r3.md) - Reactive Stack and R3 Integration *(adr)*
- [GUIDE-00001](guides\architecture-refinement-implementation-guide.md) - Architecture Refinement Implementation Guide *(guide)*
- [GUIDE-00001](guides\name-generator-enhancement-guide.md) - Name Generator Enhancement Implementation Guide *(guide)*
- [GUIDE-00001](dotnet\guides\getting-started.md) - Getting Started with Pigeon Pea .NET *(guide)*
- [GUIDE-00002](dotnet\NAVIGATION.md) - .NET Documentation Navigation Guide *(guide)*
- [GUIDE-00002](guides\dotnet-tiered-architecture-guide.md) - .NET Tiered Architecture and Layer Implementation Guide *(guide)*
- [GUIDE-00002](guides\rfc014-scene-management-adjustment-plan.md) - RFC-014 Scene Management: Adjustment Plan *(guide)*
- [GUIDE-00002](terminals\rio\CONFIG.md) - Rio Terminal Configuration — Using the Custom Font *(guide)*
- [PLAN-00003](rfcs\issues\issue-001-migrate-structure.md) - Issue #1: [RFC-005] Phase 1: Migrate project structure to new organization *(plan)*
- [PLAN-00004](rfcs\issues\issue-002-create-contracts.md) - Issue #2: [RFC-005] Phase 2: Create contract projects for plugin system *(plan)*
- [PLAN-00005](rfcs\issues\issue-003-plugin-system.md) - Issue #3: [RFC-006] Phase 1: Implement core plugin infrastructure *(plan)*
- [PLAN-00006](rfcs\issues\issue-004-game-events.md) - Issue #4: [RFC-006] Phase 2: Integrate game events with plugin system *(plan)*
- [PLAN-00007](rfcs\issues\issue-005-rendering-plugin.md) - Issue #5: [RFC-006] Phase 3: Create rendering plugin proof of concept *(plan)*
- [PLAN-00008](planning\AGENT_INFRASTRUCTURE_ISSUES.md) - GitHub Issues: Agent Infrastructure Enhancement *(plan)*

*...and 16 more*

### Draft (23)
- [](_inbox\024-nexus-camera2d-system.md) - Nexus-Camera2D: ProCamera2D-Inspired 2D Camera System *(rfc)*
- [](_inbox\025-nexus-inventory-system.md) - Nexus-Inventory: RPGCore-Inspired Inventory and Item System *(rfc)*
- [](_inbox\mcp-project-launcher-system.md) - MCP Project Launcher System *(guide)*
- [GUIDE-00002](_inbox\game-services-implementation-guide.md) - Game Services Implementation Guide: Step-by-Step Instructions for Agents *(guide)*
- [RFC-00001](rfcs\001-rendering-architecture.md) - Rendering Architecture *(rfc)*
- [RFC-00003](rfcs\003-testing-verification.md) - Testing and Verification *(rfc)*
- [RFC-00004](rfcs\004-agent-infrastructure-enhancement.md) - Agent Infrastructure Enhancement *(rfc)*
- [RFC-00012](rfcs\012-documentation-organization-management.md) - Documentation Organization Management *(rfc)*
- [RFC-00013](rfcs\013-plugin-architecture-refinement-tiered.md) - Plugin Architecture Refinement: Tier-Based System *(rfc)*
- [RFC-00014](rfcs\014-scene-management-ecs.md) - Scene Management with ECS Architecture *(rfc)*
- [RFC-00015](rfcs\015-fantasy-calendar-real-world-transformation.md) - Fantasy Calendar to Real-World Time Transformation *(rfc)*
- [RFC-00016](rfcs\016-name-generator-json-configuration.md) - Name Generator JSON Configuration System *(rfc)*
- [RFC-00017](rfcs\017-name-generator-cjk-languages.md) - Name Generator CJK Language Support *(rfc)*
- [RFC-00018](rfcs\018-name-generator-markov-chain-mode.md) - Name Generator Markov Chain Mode *(rfc)*
- [RFC-00020](rfcs\020-font-pipeline-and-pua-layout.md) - Custom CJK Monospace Game Font — Pipeline and PUA Layout *(rfc)*
- [RFC-00021](rfcs\021-terminal-distribution-and-rio-integration.md) - Terminal Distribution — Rio Integration and Bundle Layout *(rfc)*
- [RFC-00022](rfcs\022-ci-cd-font-and-bundle.md) - CI/CD for Custom Font and Developer Bundle *(rfc)*
- [RFC-00023](_inbox\game-services-architecture.md) - Game Services Architecture: Stats, Character, Avatar, Animation, Persistence, and World Management *(rfc)*
- [RFC-00023](_inbox\rfc-023-input-system-architecture-refactoring.md) - Input System Architecture Refactoring: Tier-Based System and Platform Plugins *(rfc)*
- [RFC-00024](_inbox\rfc-024-nexus-systems-architecture-refactoring.md) - Nexus Systems Architecture Refactoring: Move to Engine Layer *(rfc)*

*...and 3 more*

### Archived (1)
- [](_inbox\023-nexus-input-system.md) - Nexus-Input: Unity-Inspired Input System *(rfc)*

## Recently Updated
- **2025-11-20**: [RFC-00023](_inbox\game-services-architecture.md) - Game Services Architecture: Stats, Character, Avatar, Animation, Persistence, and World Management
- **2025-11-20**: [GUIDE-00002](_inbox\game-services-implementation-guide.md) - Game Services Implementation Guide: Step-by-Step Instructions for Agents
- **2025-11-20**: [RFC-00026](_inbox\rfc-numbering-convention.md) - RFC Numbering Convention and Document Identification
- **2025-11-20**: [RFC-00024](_inbox\stats-service-rfc.md) - Stats Service: Universal Stat Management for Game Entities
- **2025-11-20**: [RFC-00025](_inbox\world-management-service-rfc.md) - World Management Service: Multiple ECS Worlds for Interpolation, Simulation, and Scene Management
- **2025-11-19**: [GUIDE-00001](guides\architecture-refinement-implementation-guide.md) - Architecture Refinement Implementation Guide
- **2025-11-19**: [GUIDE-00002](guides\dotnet-tiered-architecture-guide.md) - .NET Tiered Architecture and Layer Implementation Guide
- **2025-11-19**: [GUIDE-00002](guides\rfc014-scene-management-adjustment-plan.md) - RFC-014 Scene Management: Adjustment Plan
- **2025-11-19**: [RFC-00013](rfcs\013-plugin-architecture-refinement-tiered.md) - Plugin Architecture Refinement: Tier-Based System
- **2025-11-19**: [RFC-00014](rfcs\014-scene-management-ecs.md) - Scene Management with ECS Architecture
- **2025-11-19**: [RFC-00015](rfcs\015-fantasy-calendar-real-world-transformation.md) - Fantasy Calendar to Real-World Time Transformation
- **2025-11-19**: [RFC-00023](_inbox\rfc-023-input-system-architecture-refactoring.md) - Input System Architecture Refactoring: Tier-Based System and Platform Plugins
- **2025-11-19**: [RFC-00024](_inbox\rfc-024-nexus-systems-architecture-refactoring.md) - Nexus Systems Architecture Refactoring: Move to Engine Layer
- **2025-11-18**: [GUIDE-00002](dotnet\NAVIGATION.md) - .NET Documentation Navigation Guide
- **2025-11-18**: [ADR-00002](dotnet\architecture\game-scale-modes.md) - Multi-Scale World & Mode System

## Most Referenced
*Documents frequently referenced by others*

- [ADR-00001](dotnet\architecture\overview.md) - Architecture Overview *(8 references)*
- [RFC-00005](rfcs\005-project-structure-reorganization.md) - Project Structure Reorganization *(8 references)*
- [RFC-00006](rfcs\006-plugin-system-architecture.md) - Plugin System Architecture *(8 references)*
- [RFC-00020](rfcs\020-font-pipeline-and-pua-layout.md) - Custom CJK Monospace Game Font — Pipeline and PUA Layout *(7 references)*
- [ADR-00003](dotnet\architecture\service-tiers.md) - Service Tiers and Category Layout *(6 references)*
- [RFC-00021](rfcs\021-terminal-distribution-and-rio-integration.md) - Terminal Distribution — Rio Integration and Bundle Layout *(6 references)*
- [GUIDE-00001](guides\architecture-refinement-implementation-guide.md) - Architecture Refinement Implementation Guide *(5 references)*
- [RFC-00013](rfcs\013-plugin-architecture-refinement-tiered.md) - Plugin Architecture Refinement: Tier-Based System *(5 references)*
- [RFC-00004](rfcs\004-agent-infrastructure-enhancement.md) - Agent Infrastructure Enhancement *(5 references)*
- [RFC-00014](rfcs\014-scene-management-ecs.md) - Scene Management with ECS Architecture *(4 references)*
- [ADR-00004](dotnet\architecture\services-and-plugins.md) - Services, Shared Libraries, and Plugins *(3 references)*
- [RFC-00016](rfcs\016-name-generator-json-configuration.md) - Name Generator JSON Configuration System *(3 references)*
- [PLAN-00008](planning\AGENT_INFRASTRUCTURE_ISSUES.md) - GitHub Issues: Agent Infrastructure Enhancement *(3 references)*

## By Topic

### Architecture (23)
- [](_inbox\023-nexus-input-system.md) - Nexus-Input: Unity-Inspired Input System
- [](_inbox\024-nexus-camera2d-system.md) - Nexus-Camera2D: ProCamera2D-Inspired 2D Camera System
- [](_inbox\025-nexus-inventory-system.md) - Nexus-Inventory: RPGCore-Inspired Inventory and Item System
- [ADR-00001](dotnet\architecture\overview.md) - Architecture Overview
- [ADR-00002](dotnet\architecture\game-scale-modes.md) - Multi-Scale World & Mode System
- [ADR-00003](dotnet\architecture\service-tiers.md) - Service Tiers and Category Layout
- [ADR-00004](dotnet\architecture\services-and-plugins.md) - Services, Shared Libraries, and Plugins
- [ADR-00005](dotnet\architecture\goap-perception-checklist.md) - GOAP Perception Checklist
- [ADR-00006](dotnet\architecture\observable-collections.md) - Observable Collections and Reactive Patterns
- [ADR-00007](dotnet\architecture\reactive-stack-and-r3.md) - Reactive Stack and R3 Integration

*...and 13 more*

### Plugins (9)
- [ADR-00001](dotnet\architecture\overview.md) - Architecture Overview
- [ADR-00003](dotnet\architecture\service-tiers.md) - Service Tiers and Category Layout
- [ADR-00004](dotnet\architecture\services-and-plugins.md) - Services, Shared Libraries, and Plugins
- [GUIDE-00002](guides\dotnet-tiered-architecture-guide.md) - .NET Tiered Architecture and Layer Implementation Guide
- [PLAN-2025-00001](rfcs\IMPLEMENTATION_PLAN.md) - Implementation Plan: Project Restructure and Plugin System
- [RFC-00005](rfcs\005-project-structure-reorganization.md) - Project Structure Reorganization
- [RFC-00006](rfcs\006-plugin-system-architecture.md) - Plugin System Architecture
- [RFC-00013](rfcs\013-plugin-architecture-refinement-tiered.md) - Plugin Architecture Refinement: Tier-Based System
- [RFC-00023](_inbox\rfc-023-input-system-architecture-refactoring.md) - Input System Architecture Refactoring: Tier-Based System and Platform Plugins

### Ecs (9)
- [ADR-00001](dotnet\architecture\overview.md) - Architecture Overview
- [GUIDE-00002](guides\rfc014-scene-management-adjustment-plan.md) - RFC-014 Scene Management: Adjustment Plan
- [RFC-00014](rfcs\014-scene-management-ecs.md) - Scene Management with ECS Architecture
- [RFC-00019](rfcs\019-nexus-gas-gameplay-ability-system.md) - Nexus-GAS: Gameplay Ability System Implementation
- [RFC-00023](_inbox\game-services-architecture.md) - Game Services Architecture: Stats, Character, Avatar, Animation, Persistence, and World Management
- [RFC-00024](_inbox\stats-service-rfc.md) - Stats Service: Universal Stat Management for Game Entities
- [RFC-00025](_inbox\world-management-service-rfc.md) - World Management Service: Multiple ECS Worlds for Interpolation, Simulation, and Scene Management
- [RFC-00027](rfcs\027-nexus-goap-ai-system.md) - Nexus-GOAP: Goal-Oriented Action Planning AI System
- [RFC-00028](rfcs\028-nexus-perception-system.md) - Nexus-Perception: Agent Perception and Awareness System

### Implementation (7)
- [GUIDE-00001](guides\architecture-refinement-implementation-guide.md) - Architecture Refinement Implementation Guide
- [GUIDE-00001](guides\name-generator-enhancement-guide.md) - Name Generator Enhancement Implementation Guide
- [GUIDE-00002](guides\rfc014-scene-management-adjustment-plan.md) - RFC-014 Scene Management: Adjustment Plan
- [GUIDE-00002](_inbox\game-services-implementation-guide.md) - Game Services Implementation Guide: Step-by-Step Instructions for Agents
- [PLAN-00008](planning\AGENT_INFRASTRUCTURE_ISSUES.md) - GitHub Issues: Agent Infrastructure Enhancement
- [PLAN-00011](planning\GITHUB_ISSUES.md) - GitHub Issues Breakdown
- [PLAN-2025-00001](rfcs\IMPLEMENTATION_PLAN.md) - Implementation Plan: Project Restructure and Plugin System

### Services (6)
- [ADR-00003](dotnet\architecture\service-tiers.md) - Service Tiers and Category Layout
- [ADR-00004](dotnet\architecture\services-and-plugins.md) - Services, Shared Libraries, and Plugins
- [GUIDE-00002](guides\dotnet-tiered-architecture-guide.md) - .NET Tiered Architecture and Layer Implementation Guide
- [GUIDE-00002](_inbox\game-services-implementation-guide.md) - Game Services Implementation Guide: Step-by-Step Instructions for Agents
- [RFC-00023](_inbox\game-services-architecture.md) - Game Services Architecture: Stats, Character, Avatar, Animation, Persistence, and World Management
- [RFC-00024](_inbox\stats-service-rfc.md) - Stats Service: Universal Stat Management for Game Entities

### Rendering (6)
- [](_inbox\024-nexus-camera2d-system.md) - Nexus-Camera2D: ProCamera2D-Inspired 2D Camera System
- [ADR-00001](dotnet\architecture\overview.md) - Architecture Overview
- [PLAN-00007](rfcs\issues\issue-005-rendering-plugin.md) - Issue #5: [RFC-006] Phase 3: Create rendering plugin proof of concept
- [PLAN-00010](planning\ENHANCEMENT_PLAN.md) - Pigeon Pea Enhancement Plan
- [RFC-00001](rfcs\001-rendering-architecture.md) - Rendering Architecture
- [RFC-00013](rfcs\013-plugin-architecture-refinement-tiered.md) - Plugin Architecture Refinement: Tier-Based System

### Library (6)
- [](_inbox\023-nexus-input-system.md) - Nexus-Input: Unity-Inspired Input System
- [](_inbox\024-nexus-camera2d-system.md) - Nexus-Camera2D: ProCamera2D-Inspired 2D Camera System
- [](_inbox\025-nexus-inventory-system.md) - Nexus-Inventory: RPGCore-Inspired Inventory and Item System
- [RFC-00019](rfcs\019-nexus-gas-gameplay-ability-system.md) - Nexus-GAS: Gameplay Ability System Implementation
- [RFC-00027](rfcs\027-nexus-goap-ai-system.md) - Nexus-GOAP: Goal-Oriented Action Planning AI System
- [RFC-00028](rfcs\028-nexus-perception-system.md) - Nexus-Perception: Agent Perception and Awareness System

### Fonts (5)
- [GUIDE-00002](terminals\rio\CONFIG.md) - Rio Terminal Configuration — Using the Custom Font
- [REFERENCE-00002](fonts\ANGLE-MAPPING-EXAMPLES.md) - Angle → Glyph Mapping Examples
- [RFC-00020](rfcs\020-font-pipeline-and-pua-layout.md) - Custom CJK Monospace Game Font — Pipeline and PUA Layout
- [RFC-00022](rfcs\022-ci-cd-font-and-bundle.md) - CI/CD for Custom Font and Developer Bundle
- [SPEC-00001](fonts\SVG-SPEC-ANGLES.md) - SVG Specification — 36 Angle Glyphs

### Refactoring (5)
- [GUIDE-00001](guides\architecture-refinement-implementation-guide.md) - Architecture Refinement Implementation Guide
- [RFC-00005](rfcs\005-project-structure-reorganization.md) - Project Structure Reorganization
- [RFC-00013](rfcs\013-plugin-architecture-refinement-tiered.md) - Plugin Architecture Refinement: Tier-Based System
- [RFC-00023](_inbox\rfc-023-input-system-architecture-refactoring.md) - Input System Architecture Refactoring: Tier-Based System and Platform Plugins
- [RFC-00024](_inbox\rfc-024-nexus-systems-architecture-refactoring.md) - Nexus Systems Architecture Refactoring: Move to Engine Layer

### Terminal (5)
- [GUIDE-00002](terminals\rio\CONFIG.md) - Rio Terminal Configuration — Using the Custom Font
- [REFERENCE-00003](terminals\rio\BUNDLE-LAYOUT.md) - Rio Bundle Layout (Template)
- [RFC-00001](rfcs\001-rendering-architecture.md) - Rendering Architecture
- [RFC-00020](rfcs\020-font-pipeline-and-pua-layout.md) - Custom CJK Monospace Game Font — Pipeline and PUA Layout
- [RFC-00021](rfcs\021-terminal-distribution-and-rio-integration.md) - Terminal Distribution — Rio Integration and Bundle Layout

### Issue (5)
- [PLAN-00003](rfcs\issues\issue-001-migrate-structure.md) - Issue #1: [RFC-005] Phase 1: Migrate project structure to new organization
- [PLAN-00004](rfcs\issues\issue-002-create-contracts.md) - Issue #2: [RFC-005] Phase 2: Create contract projects for plugin system
- [PLAN-00005](rfcs\issues\issue-003-plugin-system.md) - Issue #3: [RFC-006] Phase 1: Implement core plugin infrastructure
- [PLAN-00006](rfcs\issues\issue-004-game-events.md) - Issue #4: [RFC-006] Phase 2: Integrate game events with plugin system
- [PLAN-00007](rfcs\issues\issue-005-rendering-plugin.md) - Issue #5: [RFC-006] Phase 3: Create rendering plugin proof of concept

### Documentation (4)
- [GUIDE-00002](dotnet\NAVIGATION.md) - .NET Documentation Navigation Guide
- [REFERENCE-00001](DOCUMENTATION-SCHEMA.md) - Documentation Front-Matter Schema
- [RFC-00012](rfcs\012-documentation-organization-management.md) - Documentation Organization Management
- [RFC-00026](_inbox\rfc-numbering-convention.md) - RFC Numbering Convention and Document Identification

### Guide (4)
- [GUIDE-00001](guides\architecture-refinement-implementation-guide.md) - Architecture Refinement Implementation Guide
- [GUIDE-00001](guides\name-generator-enhancement-guide.md) - Name Generator Enhancement Implementation Guide
- [GUIDE-00002](dotnet\NAVIGATION.md) - .NET Documentation Navigation Guide
- [GUIDE-00002](_inbox\game-services-implementation-guide.md) - Game Services Implementation Guide: Step-by-Step Instructions for Agents

### Tiered-Architecture (4)
- [GUIDE-00002](guides\dotnet-tiered-architecture-guide.md) - .NET Tiered Architecture and Layer Implementation Guide
- [RFC-00013](rfcs\013-plugin-architecture-refinement-tiered.md) - Plugin Architecture Refinement: Tier-Based System
- [RFC-00023](_inbox\game-services-architecture.md) - Game Services Architecture: Stats, Character, Avatar, Animation, Persistence, and World Management
- [RFC-00023](_inbox\rfc-023-input-system-architecture-refactoring.md) - Input System Architecture Refactoring: Tier-Based System and Platform Plugins

### Name-Generation (4)
- [GUIDE-00001](guides\name-generator-enhancement-guide.md) - Name Generator Enhancement Implementation Guide
- [RFC-00016](rfcs\016-name-generator-json-configuration.md) - Name Generator JSON Configuration System
- [RFC-00017](rfcs\017-name-generator-cjk-languages.md) - Name Generator CJK Language Support
- [RFC-00018](rfcs\018-name-generator-markov-chain-mode.md) - Name Generator Markov Chain Mode

### Rfc-004 (4)
- [PLAN-00008](planning\AGENT_INFRASTRUCTURE_ISSUES.md) - GitHub Issues: Agent Infrastructure Enhancement
- [PLAN-00009](planning\CODE_REVIEW_AGENT_INFRASTRUCTURE.md) - Code Review: Agent Infrastructure Implementation
- [PLAN-00013](planning\RFC_004_EXECUTION_ORDER.md) - RFC-004 Implementation: Execution Order
- [PLAN-00014](planning\SELF_REVIEW_RFC_004_PLANNING.md) - Self-Review: RFC-004 & Issues Planning Quality

### Infrastructure (4)
- [PLAN-00004](rfcs\issues\issue-002-create-contracts.md) - Issue #2: [RFC-005] Phase 2: Create contract projects for plugin system
- [PLAN-00005](rfcs\issues\issue-003-plugin-system.md) - Issue #3: [RFC-006] Phase 1: Implement core plugin infrastructure
- [RFC-00004](rfcs\004-agent-infrastructure-enhancement.md) - Agent Infrastructure Enhancement
- [RFC-00012](rfcs\012-documentation-organization-management.md) - Documentation Organization Management

### Dotnet (3)
- [GUIDE-00001](dotnet\guides\getting-started.md) - Getting Started with Pigeon Pea .NET
- [GUIDE-00002](dotnet\NAVIGATION.md) - .NET Documentation Navigation Guide
- [GUIDE-00002](guides\dotnet-tiered-architecture-guide.md) - .NET Tiered Architecture and Layer Implementation Guide

### Pua (3)
- [REFERENCE-00002](fonts\ANGLE-MAPPING-EXAMPLES.md) - Angle → Glyph Mapping Examples
- [RFC-00020](rfcs\020-font-pipeline-and-pua-layout.md) - Custom CJK Monospace Game Font — Pipeline and PUA Layout
- [SPEC-00001](fonts\SVG-SPEC-ANGLES.md) - SVG Specification — 36 Angle Glyphs

### Cjk (3)
- [GUIDE-00001](guides\name-generator-enhancement-guide.md) - Name Generator Enhancement Implementation Guide
- [RFC-00017](rfcs\017-name-generator-cjk-languages.md) - Name Generator CJK Language Support
- [RFC-00020](rfcs\020-font-pipeline-and-pua-layout.md) - Custom CJK Monospace Game Font — Pipeline and PUA Layout
