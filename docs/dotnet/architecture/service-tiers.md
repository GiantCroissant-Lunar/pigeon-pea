...
3. **Tier 3 – Real service implementation**
   ...
   - Registered into `IRegistry` with metadata (priority, plugin id, etc.).
...
This section shows how 4 tiers and shared libs map to concrete categories.
...
- Tier‑2 proxies live next to interfaces and always delegate via `IRegistry`.
...
- **Inventory** already follows 4‑tier pattern on the game side.
...
2. Implement basic game‑level plugins for these categories using existing
