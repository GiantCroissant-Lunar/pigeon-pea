...
It complements the high-level [architecture overview](./overview.md) and focuses on runtime composition.
...
- Do **not** know about plugin system or `IRegistry`.
...
- `PigeonPea.Contracts.Input.Services.Proxy.Service` is a generated proxy that delegates to plugin registry and chooses a concrete implementation.

Contracts:

- Are intentionally **small and stable**.
- Are shared between host and plugins (loaded from same ALC).
...
- Register implementations into `IRegistry` with priorities.
...
- **How** shared libraries are wired to host environment.
...
- Implements audio service interface using LibVLC as backend.
...
For that reason, `PigeonPea.Shared.Input` is treated as part of the **app-essential** family, even though its project file currently lives under `engine/core` during migration. The plan is to move the project physically into `app-essential` once migration is stable.
