...
using HudScaleViewModel = PigeonPea.Shared.ViewModels.HudScaleViewModel;
using Terminal.Gui;
...
internal static class TerminalHudApplication
{
    ...
    private static HudScaleViewModel? _scaleViewModel;

    private static ScaleConfig CurrentScale => _scaleViewModel!.CurrentScale;
    private static double ClampZoom(double value) => Math.Clamp(value, CurrentScale.MinZoom, CurrentScale.MaxZoom);
    ...
    public static void Run()
    {
        ...
        try
        {
            ...
            // Initialize DI container for HUD-level view models and services
            var services = new ServiceCollection();
            services.AddPigeonPeaServices();
            using var provider = services.BuildServiceProvider();

            // Initialize ViewModel for color scheme management and scale/mode
            _viewModel = new MapRenderViewModel();
            _scaleViewModel = provider.GetRequiredService<HudScaleViewModel>();

            // Subscribe to scale mode changes and log them in the HUD log
            var scaleModeSubscriber = provider.GetService<ISubscriber<ScaleModeChangedEvent>>();
            if (scaleModeSubscriber != null)
            {
                scaleModeSubscriber.Subscribe(ev =>
                {
                    AppendLog(logView, $"[Scale] Mode: {ev.OldMode} -> {ev.NewMode}");
                });
            }
            ...
            var scaleItem = new Shortcut { Title = "Scale: -", Key = Key.Empty };
            var worldSizeItem = new Shortcut { Title = "World: -", Key = Key.Empty };
            ...
            status.Add(zoomItem, scaleItem, worldSizeItem, modeItem, schemeItem, brailleColorItem, overlayItem);
            top.Add(status);
            ...
            void UpdateStatus()
            {
                ...
                var kmPerCell = (CurrentScale.MetersPerCell / 1000.0) * _zoom;
                const int barCells = 10;
                var kmForBar = kmPerCell * barCells;
                var bar = new string('\u2500', barCells); // light horizontal bar
                var scaleModeName = _scaleViewModel?.Mode.ToString() ?? "?";
                scaleItem.Title = $"Scale[{scaleModeName}]: {kmPerCell:F2} km/c |{bar}| ~{kmForBar:F0} km";

                var worldWidthKm = settings.Width * (CurrentScale.MetersPerCell / 1000.0);
                var worldHeightKm = settings.Height * (CurrentScale.MetersPerCell / 1000.0);
                worldSizeItem.Title = $"World: {worldWidthKm:F0}x{worldHeightKm:F0} km";
            ...
            menu.Menus = new MenuBarItem[]
            {
                ...
                {
                    new MenuItem("Zoom _In", string.Empty, () => { _zoom = ClampZoom(_zoom * 0.8); UpdateViews(); }),
                    new MenuItem("Zoom _Out", string.Empty, () => { _zoom = ClampZoom(_zoom * 1.25); UpdateViews(); }),
                    null,
