...
public sealed class SDL3GamepadPlugin : IPlugin
{
    ...
    public Task InitializeAsync(IPluginContext context, CancellationToken ct = default)
    {
        ...
        {
            Priority = 100,
            Name = "SDL3GamepadDevice",
            Version = Version,
            PluginId = Id
        });

        _logger?.LogInformation("SDL3 gamepad input device registered successfully");
        return Task.CompletedTask;
    }

    public Task StartAsync(CancellationToken ct = default)
    {
        _logger?.LogInformation("SDL3Gamepad plugin started");
        return Task.CompletedTask;
    }

    public Task StopAsync(CancellationToken ct = default)
    {
        _logger?.LogInformation("SDL3Gamepad plugin stopped");
        _device?.Dispose();
        return Task.CompletedTask;
    }
}
