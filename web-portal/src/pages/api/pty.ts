...
export const GET: APIRoute = async ({ request, url }) => {
  ...
  const { socket, response } = globalThis.Deno?.upgradeWebSocket(request) || {
    ...
    response: new Response(
      'WebSockets require Deno or custom Node server. node-pty is currently disabled.',
      { status: 501 }
    ),
  };
  ...
  */
  ...
  socket.addEventListener('open', () => {
    ...
    socket.send(
      'NOTE: PTY integration is currently disabled due to build environment limitations.\\r\\n'
    );
