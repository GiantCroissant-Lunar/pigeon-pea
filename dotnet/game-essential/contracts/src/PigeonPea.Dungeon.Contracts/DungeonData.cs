...
public sealed class DungeonData
{
    ...
    public static DungeonData FromView(Models.DungeonView view)
    {
        var data = new DungeonData(view.Width, view.Height);
        for (int y = 0; y < view.Height; y++)
        {
            for (int x = 0; x < view.Width; x++)
            {
                if (view.Walkable[y, x]) data.SetFloor(x, y);
                else data.SetWall(x, y);

                if (data.IsOpaque(x, y) != view.Opaque[y, x])
                    data.SetOpaque(x, y, view.Opaque[y, x]);

                if (view.Doors[y, x] == 1) data.SetDoorClosed(x, y);
                else if (view.Doors[y, x] == 2) data.SetDoorOpen(x, y);
            }
        }
        return data;
    }
