...
This document captures the current design for **discrete zoom/mode levels** in Pigeon Pea and how they relate to a shared physical scale (km / m) and to technical concerns like chunking.
...
- **Scale level** – the unit resolution used for rendering in a given mode.
...
Transitions are also configurable. A transition describes how the game moves from one mode to another based on a `trigger`.
...
public sealed record CameraMode(
    ...
    string Environment,           // derived from the scale
...
- We have chosen a **physically meaningful scale**:
  - `1 world unit = 1 km` for the FMG map.
...
- Overlays (world & dungeon) become more meaningful when annotated with physical metadata based on the scale.
...
- Implement the transition system that moves the player between `world`, `town`, `dungeon-coarse`, `dungeon-fine`, `interior`, and `vehicle` modes.
