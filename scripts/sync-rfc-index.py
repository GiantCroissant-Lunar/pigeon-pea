...
def analyze_rfc_indexing(rfcs_dir: Path) -> Dict:
    ...
    if rfcs:
        filename_numbers = sorted([r["filename_number"] for r in rfcs if r["filename_number"]])
        doc_id_numbers = sorted([r["doc_id_number"] for r in rfcs if r["doc_id_number"]])
        ...
        for i in range(len(filename_numbers) - 1):
            if filename_numbers[i+1] - filename_numbers[i] > 1:
                issues["gaps"].append({
                    ...
                    "missing": list(range(filename_numbers[i] + 1, filename_numbers[i+1])),
                })
        ...
        for i in range(len(doc_id_numbers) - 1):
            if doc_id_numbers[i+1] - doc_id_numbers[i] > 1:
                issues["gaps"].append({
                    ...
                    "missing": list(range(doc_id_numbers[i] + 1, doc_id_numbers[i+1])),
                })
    ...
    for rfc in rfcs:
        if rfc["filename_number"]:
            filename_counts[rfc["filename_number"]] = filename_counts.get(rfc["filename_number"], 0) + 1
        if rfc["doc_id_number"]:
            doc_id_counts[rfc["doc_id_number"]] = doc_id_counts.get(rfc["doc_id_number"], 0) + 1
    
    for num, count in filename_counts.items():
        if count > 1:
            issues["duplicates"].append({"type": "filename", "number": num, "count": count})
    
    for num, count in doc_id_counts.items():
        if count > 1:
            issues["duplicates"].append({"type": "doc_id", "number": num, "count": count})
...
def generate_sync_report(analysis: Dict) -> str:
    ...
    if issues["mismatches"]:
        ...
        report.append("| Filename | Filename # | Doc ID | Doc ID # | Recommended Action |\n")
        report.append("|----------|------------|--------|----------|--------------------|\n")
    ...
    if issues["missing_filename_number"]:
        report.append(f"\n## ❌ Missing Filename Number ({len(issues['missing_filename_number'])})\n")
    ...
    if issues["duplicates"]:
        ...
        for dup in issues["duplicates"]:
            report.append(f"- {dup['type'].title()} #{dup['number']} appears {dup['count']} times\n")
...
def generate_prd_rfc_schema() -> str:
    ...
    schema.append("\n## Overview\n")
    schema.append("Product Requirement Documents (PRDs) and RFCs serve different purposes:\n\n")
    schema.append("- **PRD**: What to build and why (product perspective)\n")
    schema.append("- **RFC**: How to build it (technical perspective)\n\n")
    schema.append("**Recommended Approach**: PRDs reference RFCs (one-to-many relationship)\n\n")
    ...
    schema.append("- **Use Case**: Track which RFCs implement a product requirement\n\n")
    ...
    schema.append("    PRD1[PRD: Fantasy Calendar] --> RFC15[RFC-015: Time Transform]\n")
    ...
    schema.append("\n## Benefits\n")
    schema.append("1. **Traceability**: Track from product requirement to technical implementation\n")
    schema.append("2. **Impact Analysis**: See which RFCs are affected by PRD changes\n")
    schema.append("3. **Completeness**: Verify all PRD requirements have RFC coverage\n")
    schema.append("4. **Context**: Understand product rationale behind technical decisions\n")
