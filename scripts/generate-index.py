...
def collect_documents(docs_dir: Path) -> List[Dict]:
    ...
    for md_file in docs_dir.rglob("*.md"):
        ...
        if any(str(rel_path).startswith(pattern) for pattern in ["index/", "archive/", "_inbox/"]):
            continue
...
def generate_index(documents: List[Dict], docs_dir: Path) -> str:
    ...
    type_order = ["rfc", "guide", "adr", "plan", "spec", "reference", "finding", "glossary"]
    type_names = {
    ...
    for doc_type in type_order:
        ...
        docs = sorted(by_type[doc_type], key=lambda d: d["front_matter"].get("doc_id", ""))
    ...
    for status in status_order:
        ...
        docs = sorted(by_status[status], key=lambda d: d["front_matter"].get("doc_id", ""))
    ...
    recently_updated = sorted(
        docs_with_dates,
        key=lambda d: str(d["front_matter"].get("updated") or d["front_matter"].get("created", "")),
        reverse=True,
    )[:15]
    ...
    if ref_counts:
        most_referenced = sorted(ref_counts.items(), key=lambda x: x[1], reverse=True)[:15]
        ...
        for doc_id, count in most_referenced:
            # Find the document
            doc = next((d for d in documents if d["front_matter"].get("doc_id") == doc_id), None)
            if doc:
                title = doc["front_matter"].get("title", "Untitled")
                rel_path = doc["rel_path"]
                index.append(f"- [{doc_id}]({rel_path}) - {title} *({count} references)*\n")
    ...
    if all_tags:
        ...
        for tag, docs in sorted_tags[:20]:  # Top 20 tags
            ...
            for doc in sorted(docs, key=lambda d: d["front_matter"].get("doc_id", ""))[:10]:
                fm = doc["front_matter"]
                doc_id = fm.get("doc_id", "N/A")
                title = fm.get("title", "Untitled")
                rel_path = doc["rel_path"]
...
def generate_directory_readme(directory: Path, documents: List[Dict], docs_dir: Path) -> str:
    """Generate README for a specific directory."""
    # Filter documents in this directory
    dir_docs = [
        d for d in documents
        if d["path"].parent == directory
    ]
