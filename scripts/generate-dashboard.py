...
def generate_dashboard(rfcs: List[Dict]) -> str:
    ...
    active = [r for r in rfcs if r["front_matter"].get("implementation", {}).get("status") == "in-progress"]
    if active:
        dashboard.append("\n## 🚀 Active RFCs\n")
    ...
    # Blocked RFCs
    blocked = [r for r in rfcs if r["front_matter"].get("implementation", {}).get("status") == "blocked"]
    if blocked:
        dashboard.append("\n## ⚠️ Blocked RFCs\n")
    ...
    # Recently completed
    completed = [r for r in rfcs if r["front_matter"].get("implementation", {}).get("status") == "completed"]
    ...
    # Not started
    not_started = [r for r in rfcs if r["front_matter"].get("implementation", {}).get("status", "not-started") == "not-started"]
    if not_started:
        dashboard.append(f"\n## 📋 Not Started ({len(not_started)} RFCs)\n")
        dashboard.append("| RFC | Title | Status |\n")
        dashboard.append("|-----|-------|--------|\n")
        
        for rfc in sorted(not_started, key=lambda r: r["front_matter"].get("doc_id", ""))[:10]:
            fm = rfc["front_matter"]
            doc_id = fm.get("doc_id", "N/A")
            title = fm.get("title", "Untitled")
    ...
    rfcs_with_dates = [
        r for r in rfcs
        if r["front_matter"].get("implementation", {}).get("started")
    ]
    if rfcs_with_dates:
        dashboard.append("\n## 📅 Implementation Timeline\n")
        dashboard.append("```mermaid\n")
        ...
        for rfc in sorted(
            rfcs_with_dates,
            key=lambda r: r["front_matter"].get("implementation", {}).get("started", ""),
        ):
            fm = rfc["front_matter"]
            impl = fm.get("implementation", {})
            doc_id = fm.get("doc_id", "N/A")
