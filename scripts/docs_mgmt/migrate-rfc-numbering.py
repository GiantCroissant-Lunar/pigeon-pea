...
def migrate_file(
    ...
    try:
        ...
        if "implements" in front_matter and front_matter["implements"]:
            front_matter["implements"] = update_doc_id_format(
                front_matter["implements"]
            )
...
def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Migrate RFC numbering to new format")
    parser.add_argument(
        "--docs-dir",
        type=Path,
    ...
    if args.apply:
        ...
        print(
            ...
            "git commit -m 'docs: migrate to RFC-00026 numbering format'"
        )
    else:
