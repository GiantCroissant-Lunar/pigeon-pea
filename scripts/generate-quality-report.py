...
def calculate_completeness_score(front_matter: Dict) -> int:
    ...
    # Count filled fields
    filled = sum(
        1 for field in all_fields if field in front_matter and front_matter[field]
    )
    ...
    required_filled = sum(
        1 for field in required_fields if field in front_matter and front_matter[field]
    )

    # Score: 70% for required, 30% for optional
    required_score = (required_filled / len(required_fields)) * 70
    optional_score = (
        (filled - required_filled) / (len(all_fields) - len(required_fields))
    ) * 30
...
def calculate_clarity_score(front_matter: Dict, content: str) -> int:
    ...
    if summary:
        ...
        if any(
            ...
            for term in ["implement", "design", "architecture", "system"]
        ):
            score += 10
...
def calculate_quality_score(
    ...
    overall = int(completeness * 0.3 + freshness * 0.2 + linkage * 0.2 + clarity * 0.3)
...
def generate_quality_report(
    ...
    avg_score = (
        int(sum(r["quality"]["overall"] for r in quality_results) / total)
        if total > 0
        else 0
    )
    ...
    report.append(
        f"- **Needs Improvement** (<60): {by_grade.get('needs_improvement', 0)}\n"
    )
    ...
    needs_improvement = [
        r for r in quality_results if r["quality"]["grade"] == "needs_improvement"
    ]
